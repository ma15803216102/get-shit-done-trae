...
```bash
npm view get-shit-done-trae version 2>/dev/null
```
...
To update manually: `npx get-shit-done-trae --global`
```
...
**If LOCAL install:**
```bash
npx -y get-shit-done-trae@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y get-shit-done-trae@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y get-shit-done-trae@latest --claude --global
```
